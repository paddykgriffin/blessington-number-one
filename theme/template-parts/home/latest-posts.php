                ...
                <?php while ($the_query->have_posts()):
                    ...
                    <div class="post-wrapper flex flex-col border-gray-200 border shadow-lg">
                        ...
                        <div class="px-4 py-6 bg-background">
                            ...
                            <p class="pb-3"><?php echo get_the_excerpt(); ?></p>
                            <div class="flex justify-between items-center">
                                ...
                                if ($categories) {
                                    foreach ($categories as $category) {
                                        $output .= '<a class="text-[14px] flex items-center" href="' . get_category_link($category->term_id) . '">
                                                <span class="material-symbols-outlined text-[10px] text-(--no1-red)">&#xe866;</span>' . $category->name . '</a>' . $separator;
                                    }
                                    echo trim($output, $separator);
                                }
                                ?>
                                <a href="<?php echo esc_url(get_permalink($post->ID)); ?>" title="Read more about this post"
                                    class="text-[14px] flex items-center font-medium hover:text-(--no1-green) transition-all duration-300 hover:px-3">Read
                                    More<span class="material-symbols-outlined !text-[16px] ml-1">&#xe5c8;</span></a>
                            </div>
                        </div>
                    </div>
                <?php endwhile; ?>
